...
import { useAnimalSales, AnimalSale, AnimalSaleItem, getDefaultSale } from "@/hooks/useAnimalSales";
...
interface AnimalSaleFormProps {
  existingSale?: AnimalSale;
  onBack: () => void;
  onSaved?: () => void;
}

export function AnimalSaleForm({ existingSale, onBack, onSaved }: AnimalSaleFormProps) {
  const { user } = useAuth();
  const { farm } = useFarm();
  ...
  const [items, setItems] = useState<AnimalSaleItem[]>(existingSale?.items || []);
  const [selectedAnimalIds, setSelectedAnimalIds] = useState<string[]>(
    existingSale?.items?.map((i) => i.animal_id) || []
  );
  ...
  const updateSale = (updates: Partial<AnimalSale>) => {
  ...
  const handleSaveDraft = async () => {
    ...
    if (savedSale) {
      ...
      toast({ title: "Draft Saved", description: `Sale ${savedSale.sale_number} saved successfully.` });
      onSaved?.();
    }
    setSaving(false);
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold font-display text-foreground">
                {existingSale ? "Edit Sale" : "New Animal Sale"}
              </h1>
              {sale.sale_number && (
      ...
      {/* Section B & C: Seller & Buyer Details */}
